...
def loadCalEarly():
    ...
    skipDays = 0
    numDays = 225 #from start to end of first wave
...
ax.set_title("Tracking Transmission Rate in Italy (First Wave)", fontsize = 35)

fig2, ax2 = plt.subplots(figsize=(18,8))
ax2.set_title("Optimal Model Predictions in Italy", fontsize = 35)

#set up params
sird.weightDecay= .97
sird.regularizer=10

sird_fd.weightDecay = .97
...
ax2.plot(infect, color="red", label="Actual")
ax2.plot(ipConst, color="blue", label="Constant", linestyle="dotted")
ax2.plot(ipFeed, color="green", label="Feedback", linestyle="dashed")

ax2.axvline(len(suscept)-dTP, color='black', linestyle='dashed', label="Prediction")

#Customizing the Figure
ax2.tick_params(axis="both", labelsize=20)

ax2.set_xlabel("Time in Days", fontsize = 30)
ax2.set_ylabel("Infected", fontsize = 30)

ax2.set_ylim([0, max(infect)*1.2])
