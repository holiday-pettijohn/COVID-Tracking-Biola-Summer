...
def getMatrix(S, I, R, D, V, nonLinVars):    
    ...
    nextIterMatrix[:,0,0] = S[1:] - S[:-1] + ( (V[1:] - V[:-1]) * S[:-1] / (S[:-1] + R[:-1]) ) 
    nextIterMatrix[:,1,0] = I[1:] - I[:-1]
    nextIterMatrix[:,2,0] = R[1:] - R[:-1] + ( (V[1:] - V[:-1]) * R[:-1] / (S[:-1] + R[:-1]) ) 
...
def getLinVars(S, I, R, D, V, nonLinVars, graph=False):
    nu = getNu(I,D)
    gamma = getGamma(S, I,R, V)
    beta0, beta1 = getBeta(S,I,R,D, V, nonLinVars, gamma,nu)
...
def getGamma(S, I, R, V):
    ...
    # R(t+1) - R(t) = gamma*I(t)
    y[:,0] = R[1:] - R[:-1] + ( (V[1:] - V[:-1]) * R[:-1] / (S[:-1] + R[:-1]) ) 
    x[:,0] = I[:-1]
...
def getBeta(S,I,R,D, V, nonLinVars, gamma, nu):
    ...
    pop = S+I+R+D+V #for normalizing b1*I

    shiftI = np.zeros(len(I))
    shiftI[delay:] = I[:-delay]

    #S and I rows
    y[::2,  0] = (S[1:] - S[:-1]) + ( (V[1:] - V[:-1]) * S[:-1] / (S[:-1] + R[:-1]) ) #::2 is for skipping every other row (starts at 0)
...
def getError(S, I, R, D, V, linVars, nonLinVars, regError=True): #the custom error function for SIRD    
    y, x = getMatrix(S, I, R, D, V, nonLinVars)
...
def getBetaTime(S, I, R, D, V, linVars, nonLinVars): #calculate beta from b0, b1, b2, b3
...
def gridNonLinVars(S,I,R,D,V, constraints, varResols): #solve for non linear vars, q, b1, b2, b3
    ...
    linVars = getLinVars(S, I, R, D,V, minVars)
    minCost = getError(S, I, R, D,V, linVars, minVars) #the custom error function for SIRD
    ...
    while(continueLoop):
        linVars = getLinVars(S, I, R, D,V, currVars)
        currCost = getError(S, I, R, D,V, linVars, currVars) #error function for SIRD
        if(currCost < minCost):
            minCost = currCost
    ...
    linVars = getLinVars(S, I, R, D,V, minVars) #set lin vars according to the min nonlin vars
    return linVars, minVars #return vars and linVars

def solveAllVars(S, I, R, D, V, nonLinConstraints, nonLinResol, printOut=False):
    linVars, nonLinVars = gridNonLinVars(S,I,R,D,V, nonLinConstraints, nonLinResol)
    ...
    if(printOut):
        ...
        print("cost: ", getError(S,I,R,D,V, linVars, nonLinVars) )
        print() #spacer
...
#--------------------------------------------------------------------------  
def displayData(S,I,R,D,V, graphVals=[False,True,True,True]):
...
def calculateFuture(S,I,R,D,V, daysToPredict, params=None, nonLinParams=None):
    if(nonLinParams==None): #nonlinParams not calculate them
        varRanges = [(0,5000), (0,10)]
        varResol = [100, 10]
        params, nonLinParams = solveAllVars(S,I,R,D,V[:len(S)], varRanges, varResol)

    if(params==None): #nonLinParams are set but params aren't
        params=getLinVars(S,I,R,D,V[:len(S)], nonLinVars)

    print("Non Lin Vars:", nonLinParams)
    print("Lin Vars:", params)

    #set up matrices and starting info
    dt, X = getMatrix(S,I,R,D,V[:len(S)], nonLinParams)
    ...
    for t in range(T, T + daysToPredict): #go from last element in known list to end of prediction, see paper for method
        pop = SP[t] + IP[t] + RP[t] + DP[t] + V[t] #for normalizing b2*I
    ...
    for t in range(T):
        pop = S[t] + I[t] + R[t] + D[t] + V[t] #for normalizing b2*I
...
def predictFuture(S,I,R,D,V, daysToPredict, linVars=None, nonLinVars=None, graphVals=[False,True,True,True], graph=True):
    pS, pI, pR, pD = calculateFuture(S,I,R,D,V, daysToPredict, params=linVars, nonLinParams=nonLinVars)
...
def predictMatch(S,I,R,D,V, daysToPredict, linVars=None, nonLinVars=None, graphVals=[False,True,True,True], graph=True):
    pS, pI, pR, pD = calculateFuture(S[0:-daysToPredict], I[0:-daysToPredict], R[0:-daysToPredict], D[0:-daysToPredict], V, daysToPredict, params=linVars, nonLinParams=nonLinVars)
